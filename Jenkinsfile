...
pipeline {
  ...
  environment {
     APP_NAME = 'BuyRequest/buy-request-dashboard'
     WORKSPACE = '/home/Jenkins/workspace/BuyRequest/buy-request-dashboard'
  }
   agent none
   stages {
            ...
            steps {
                script {
                    sshagent (credentials: ['buyrequestdev']) {
                        ...
                        && cat .env \
                        && cd ../buy-request-dashboard && git checkout -f && git pull \
                        && cd /home/ubuntu/buy-request/docker && make dashboard-restart && make nginx-restart \
                        && make dashboard-logs-without-tail \
